"""Kaggriculture-AgriSchemer-V2 (Apex Grandmaster): 8C/5S Choreography with Instant Liquidity Sweep, Market Order Prioritization, and Town-Shop Aware Arbitrage."""
import copy
import json
import base64
import zlib

# Embedded Master Replay Trace
_MASTER_B85 = (
...
_MASTER_TRACE = json.loads(zlib.decompress(base64.b85decode(_MASTER_B85)).decode("utf-8"))

_BASE_PRICE = {
    "MELON": 250, "STRAWBERRY": 120, "MILK": 160, "WOOL": 200,
    "WHEAT": 25, "CARROT": 35, "TOMATO": 60, "EGG": 50, "FERTILIZER": 100
}

_PRIORITY_ORDER = {
    "MELON": 1, "WOOL": 2, "MILK": 3, "STRAWBERRY": 4,
    "FERTILIZER": 5, "CARROT": 6, "TOMATO": 7, "WHEAT": 8, "EGG": 9
}
...
def _optimize_market_orders(action, obs, step):
    """
    1. Instant Liquidity Sweeper: Sells high-margin inventory as soon as it arrives in shed.
    2. Front-running Snipe: Front-runs scheduled batch sales when playing against clones.
    3. Terminal Step 700-720 full liquidation.
    4. Strict Order Prioritization: Sorts SELL orders to the very top of market actions.
    """
    ...
    # Track existing sells in this step's trace
    existing_sells = {}
    other_orders = []
    
    for order in raw_orders:
        if isinstance(order, list) and len(order) >= 3 and order[0] == "SELL":
            existing_sells[order[1]] = existing_sells.get(order[1], 0) + max(0, int(order[2] or 0))
        else:
            other_orders.append(order)

    # Generated dynamic sell orders
    dyn_sells = {}

    # 1. Instant Shed Sweep for High-Value Goods
    # If we have premium items in shed, sell them if price >= 40% of base
    for item in ("MELON", "STRAWBERRY", "MILK", "WOOL"):
        count = shed.get(item, 0)
        cur_p = prices.get(item, 1)
        base_p = _BASE_PRICE.get(item, 100)
        
        # In early game (day < 11), keep melon sales on master schedule; after day 10, sweep immediately
        if item == "MELON" and step < 260:
            continue
            
        if count > 0 and cur_p >= (base_p * 0.40):
            dyn_sells[item] = count

    # 2. Front-Running Upcoming Master Trace Sales if Clone Detected
    lookahead = 3 if _CLONE_CONFIDENCE >= 2 else 1
        ...
        for f_order in _MASTER_TRACE[f_step].get("market", []) or []:
            if isinstance(f_order, list) and len(f_order) >= 3 and f_order[0] == "SELL":
                ...
                if item in ("MELON", "STRAWBERRY", "MILK", "WOOL"):
                    ...
                    cur_p = prices.get(item, 1)
                    base_p = _BASE_PRICE.get(item, 100)
                    if count > 0 and cur_p >= (base_p * 0.35):
                        dyn_sells[item] = max(dyn_sells.get(item, 0), min(count, qty))

    # 3. Terminal Endgame Sweep (Steps 700-720)
    if step >= 700:
        for item in ("MELON", "STRAWBERRY", "WOOL", "MILK", "FERTILIZER", "WHEAT", "CARROT", "TOMATO", "EGG"):
            count = shed.get(item, 0)
            if count > 0:
                dyn_sells[item] = count

    # Combine trace sells and dynamic sells
    final_sells = {}
    for item, qty in existing_sells.items():
        final_sells[item] = max(final_sells.get(item, 0), qty)
    for item, qty in dyn_sells.items():
        final_sells[item] = max(final_sells.get(item, 0), qty)

    # Format Sell orders sorted by commodity priority
    sorted_sell_items = sorted(final_sells.keys(), key=lambda k: _PRIORITY_ORDER.get(k, 99))
    sell_orders = [["SELL", item, final_sells[item]] for item in sorted_sell_items if final_sells[item] > 0]

    # Combine: SELL orders ALWAYS first, then other orders (HIRE, BUY_SEED, BUY_ANIMAL, etc.)
    combined_orders = (sell_orders + other_orders)[:10]
    action["market"] = combined_orders
...
def agent(obs):
    ...
    _optimize_market_orders(step_action, obs, step)
