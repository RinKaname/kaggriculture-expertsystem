...
class ApexDominatorAgent:
    def __init__(self):
        self.p = {
            "sell_thresh": 0.65,
            "milk_wool_thresh": 0.55,
            "sell_batch_size": 6,
            "shop_demand_thresh_mult": 0.85,
            "shop_batch_bonus": 4,
    ...
    def act(self, obs):
        ...
        # --- 1. Continuous Market Selling (Revenue Sorted) ---
        ...
        if len(unlocked_quads) == 1 and money >= 1050 and day <= self.p["quad2_day_cutoff"] and len(market_orders) < 10:
            ...
            market_orders.append(["BUY_LAND"])
        elif self.p["quad4_enable"] and len(unlocked_quads) == 3 and money >= 4100 and day <= self.p["quad4_day_cutoff"] and len(market_orders) < 10:
            market_orders.append(["BUY_LAND"])
        ...
        safety_margin = 150 + (1000 if len(unlocked_quads) == 1 and day >= 6 else (2000 if len(unlocked_quads) == 2 and day >= 10 else 0))
...
def agent(obs):
    ...
    if obs.get("step", 0) == 0 or p_id not in _player_agents:
        _player_agents[p_id] = ApexDominatorAgent()
