"""
Apex Grandmaster Strategy V3 (110k-135k Meta):
- Animal Hub + Daily Fertilizer Arb + High Yield Strawberry/Melon Matrix.
- Workers use harvested fertilizer on Strawberry & Melon crops (FERTILIZE).
- Dedicated morning pasture feed/care sweep so animals NEVER miss a day.
- Dynamic paced selling and end-of-season dump.
"""
import math
...
class ApexGrandmasterAgentV3:
    ...
    def act(self, obs):
        ...
        # Scaling workforce
        needed_workers = max(6, math.ceil(num_tiles / 4.8) + (2 if active_animals > 0 else 0))
        ...
        if hour == 0:
            ...
            elif day in [4, 5, 6, 7, 8, 9, 10, 11] and money >= 900 and active_animals < len(target_pastures):
            ...
            if 4 <= day <= 16 and avail_money >= 200:
                straw_target = 42 if num_tiles >= 75 else (24 if num_tiles >= 50 else 6)
                if active_strawberries < straw_target:
                    straw_buy = min(seeds_needed, straw_target - active_strawberries, int((avail_money * 0.6) // 100))
            ...
            if day <= 18 and avail_money >= 160:
                melon_target = 25 if num_tiles >= 75 else (14 if num_tiles >= 50 else 7)
                if active_melons < melon_target:
                    melon_buy = min(seeds_needed, melon_target - active_melons, int((avail_money * 0.6) // 80))
        ...
        is_emergency_dump = (total_shed_items >= 75) or (day >= 28) or (money < 150 and day < 14)
        ...
        for item, count in shed.items():
            ...
            thresh = 0.45 if item in ["FERTILIZER", "MILK", "WOOL"] else 0.60
            if is_emergency_dump or cur_p >= (base_p * thresh):
                sell_qty = count if is_emergency_dump else min(count, 5)
        ...
        if current_tile != "LOCKED" and (cx, cy) not in target_pastures:
            if isinstance(current_tile, dict):
                ...
                if kind == "WEED":
                    ...
                    # Fertilize Strawberry / Melon if carrying fertilizer and not yet fertilized
...
def agent(obs):
    ...
    if _agent_inst is None or obs.get("step", 0) == 0:
        _agent_inst = ApexGrandmasterAgentV3()
