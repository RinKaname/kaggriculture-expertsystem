"""
Apex Grandmaster Strategy V4 (120k-135k Meta):
- Animal Hub + Daily Fertilizer Cash Injection ($400/day early game).
- Guaranteed liquidity buffer (> $250) before buying seeds or animals.
- Phase 1 (D0-D6): 3 Cows + 1 Sheep + 6 Melons + 11 Wheat. Sell all fertilizer for instant capital.
- Phase 2 (D7-D14): Unlock NE Land ($1k) -> Add 6 Pastures (3 Cows, 3 Sheep) -> Plant Strawberries + Melons.
- Phase 3 (D15-D27): Unlock SW & SE Land -> 14 Animals + 40 Strawberries + 12 Melons + Paced Selling.
- Phase 4 (D28-D29): Full Shed Liquidation.
"""
import math
...
SHED_TILES = [(4, 4), (4, 3), (3, 4), (5, 4), (4, 5)]
...
class ApexGrandmasterAgentV4:
    ...
    def act(self, obs):
        ...
        # Scaling workforce (minimum 5, scales with land and animals)
        needed_workers = max(5, math.ceil(num_tiles / 5.2) + (2 if active_animals > 0 else 0))
        target_hires = needed_workers - 1

        # --- Continuous Selling Throughout the Day (Immediate Liquidity) ---
        total_shed_items = sum(shed.values())
        is_emergency_dump = (total_shed_items >= 70) or (day >= 28) or (money < 150)
        ...
        for item, count in shed.items():
            ...
            # Fertilizer sells aggressively in early game (Day 0-7) to provide huge capital
        ...
        # --- Hour 0: Macro Economic Planning & Purchases ---
        if hour == 0:
            # 1. Land Expansion (NE at Day 6-8, SW at Day 9-12, SE at Day 12-16)
            if len(unlocked_quads) == 1 and money >= 1050 and day <= 8:
                market_orders.append(["BUY_LAND"])
            elif len(unlocked_quads) == 2 and money >= 2050 and day <= 12:
                market_orders.append(["BUY_LAND"])
            elif len(unlocked_quads) == 3 and money >= 4050 and day <= 16:
                market_orders.append(["BUY_LAND"])
            ...
            # 3. Market Feed Refill for Animals (Always maintain safety stock)
            if active_animals > 0:
                wheat_needed = max(0, (active_animals * 3) - shed.get("WHEAT", 0))
            ...
            if day == 0 and money >= 1800:
                ...
                market_orders.append(["BUY_PRODUCT", "WHEAT", 16])
                market_orders.append(["BUY_SEED", "MELON", 6])
                market_orders.append(["BUY_SEED", "WHEAT", 11])
            elif day in [5, 6, 8, 9, 10, 11] and money >= 1200 and active_animals < len(target_pastures):
            ...
            avail_money = money - land_reserved - 150
            ...
            # Strawberry Matrix (Day 4 - 16)
            if 4 <= day <= 16 and avail_money >= 200:
                straw_target = 40 if num_tiles >= 75 else (22 if num_tiles >= 50 else 6)
                if active_strawberries < straw_target:
                    straw_buy = min(seeds_needed, straw_target - active_strawberries, int((avail_money * 0.5) // 100))
            ...
            # Melon Engine (Day 0 - 18)
            if day <= 18 and avail_money >= 160:
                melon_target = 25 if num_tiles >= 75 else (14 if num_tiles >= 50 else 6)
            ...
            # Supporting Wheat & Liquidity Carrots
            if day < 27 and seeds_needed > 0 and avail_money >= 10:
                wheat_buy = min(seeds_needed, int(avail_money // 10))
                if wheat_buy > 0 and len(market_orders) < 10:
                    market_orders.append(["BUY_SEED", "WHEAT", wheat_buy])
                    seeds_needed -= wheat_buy
                    avail_money -= wheat_buy * 10
    ...
    def _get_worker_action(self, pos, assigned_tiles, target_pastures, farm, private, day, hour, inv, worker_idx):
        ...
        # Phase 5: Check if any pasture needs tending (dedicated morning sweep)
        if hour <= 12:
            for (px, py) in target_pastures:
                if px < len(farm["tiles"][0]) and py < len(farm["tiles"]):
                    t = farm["tiles"][py][px]
        ...
        has_produce = any(inv.get(it, 0) > 0 for it in ["MILK", "WOOL", "FERTILIZER", "STRAWBERRY", "MELON", "CARROT", "TOMATO", "WHEAT"])
...
def agent(obs):
    ...
    if _agent_inst is None or obs.get("step", 0) == 0:
        _agent_inst = ApexGrandmasterAgentV4()
