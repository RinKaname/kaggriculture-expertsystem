...
class EliteFarmAgent:
    ...
    def act(self, obs):
        ...
        # Workers: 1 worker per 7 tiles
        needed_workers = max(1, math.ceil(num_tiles / 7.0))
        target_hires = needed_workers - 1

        # --- Hour 0: Daily Strategic Planning & Purchasing ---
        if hour == 0:
            # 1. Rapid Land Expansion (Aggressive reinvestment)
            if unlocked_quads == 1 and money >= 1200 and day <= 22:
                market_orders.append(["BUY_LAND"])
            elif unlocked_quads == 2 and money >= 2500 and day <= 18:
                market_orders.append(["BUY_LAND"])
            ...
            land_cost_reserved = 1000 if ("BUY_LAND" in [o[0] for o in market_orders] and unlocked_quads == 1) else (
            ...
            if seeds_needed > 0 and avail_money > 20:
                if day >= 27:
                    ...
                    melon_target = 15 if num_tiles >= 50 else 8
                    melon_buy = 0
                    if active_melons < melon_target and avail_money >= 160:
                        melon_buy = min(seeds_needed, melon_target - active_melons, int((avail_money * 0.45) // CROPS["MELON"]["seed"]))
                    ...
                    if seeds_needed > 0 and avail_money >= CROPS["WHEAT"]["seed"] and len(market_orders) < 10:
                    ...
                    if carrot_buy > 0 and len(market_orders) < 10:
                        market_orders.append(["BUY_SEED", "CARROT", carrot_buy])
                    elif avail_money >= CROPS["WHEAT"]["seed"] and len(market_orders) < 10:
                        wheat_buy = min(seeds_needed, int(avail_money // CROPS["WHEAT"]["seed"]))
                        if wheat_buy > 0:
                            market_orders.append(["BUY_SEED", "WHEAT", wheat_buy])
