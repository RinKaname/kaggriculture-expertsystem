"""
Exact Grandmaster Replay Architecture (110k-135k Meta):
- Land Expansion without rigid day cutoffs:
  - NW (Day 0) -> NE when money >= $1,050 -> SW when money >= $2,100 -> SE when money >= $4,200
- Animal Engine:
  - Day 0-3: 3 Cows + 1 Sheep
  - Day 6-10: Scale to 6 Cows + 6 Sheep
  - Day 11+: Scale to 8 Cows + 6 Sheep (14 total)
- Crop Engine:
  - Day 0: 8 Melons + 11 Wheat
  - Day 5-15: Aggressively scale to 40 Strawberries + 10 Melons
  - Day 26+: Replace expiring crops with Wheat
- Shed-Adjacent Pastures + Morning Feed Priority (0 starve rate)
"""
import math
...
class ExactGrandmasterAgent:
    ...
    def act(self, obs):
        ...
        is_emergency_dump = (total_shed_items >= 65) or (day >= 28) or (money < 150 and day < 14)
        ...
        for item, count in shed.items():
            ...
            if is_emergency_dump or cur_p >= (base_p * thresh):
                sell_qty = count if (is_emergency_dump or item == "FERTILIZER" or day >= 28) else min(count, 5)
                if sell_qty > 0 and len(market_orders) < 10:
                    market_orders.append(["SELL", item, sell_qty])

        # --- Land Expansion (Dynamic) ---
        if len(unlocked_quads) == 1 and money >= 1050 and day <= 10 and len(market_orders) < 10:
            market_orders.append(["BUY_LAND"])
        elif len(unlocked_quads) == 2 and money >= 2050 and day <= 16 and len(market_orders) < 10:
            market_orders.append(["BUY_LAND"])
        elif len(unlocked_quads) == 3 and money >= 4100 and day <= 20 and len(market_orders) < 10:
            market_orders.append(["BUY_LAND"])
        ...
        # --- Livestock Scaling ---
        max_target_animals = 4 if len(unlocked_quads) == 1 else (12 if len(unlocked_quads) == 2 else 14)
        if total_animals < max_target_animals and money >= 800 and day <= 14:
        ...
        # 1. Strawberry Matrix (Days 4 - 16) -> Priority 1
        straw_target = 40 if num_tiles >= 75 else (20 if num_tiles >= 50 else 4)
        if 4 <= day <= 16 and (active_strawberries + held_straw_seeds) < straw_target and spendable_money >= 100:
            s_buy = min(straw_target - (active_strawberries + held_straw_seeds), 4, int(spendable_money // 100))
            if s_buy > 0 and len(market_orders) < 10:
                market_orders.append(["BUY_SEED", "STRAWBERRY", s_buy])
                spendable_money -= s_buy * 100

        # 2. Melon Engine (Days 0 - 18) -> Priority 2
        melon_target = 10 if num_tiles <= 50 else 14
        if day <= 18 and (active_melons + held_melon_seeds) < melon_target and spendable_money >= 80:
            m_buy = min(melon_target - (active_melons + held_melon_seeds), 4, int(spendable_money // 80))
            if m_buy > 0 and len(market_orders) < 10:
                market_orders.append(["BUY_SEED", "MELON", m_buy])
                spendable_money -= m_buy * 80

        # 3. Wheat Supporting Crop -> Priority 3
    ...
    def _get_worker_action(self, pos, assigned_tiles, target_pastures, farm, private, day, hour, inv, worker_idx):
        ...
        # Phase 5: Morning Pasture Sweep (Priority for workers 0 and 1)
        if hour <= 12 and worker_idx in [0, 1]:
            for (px, py) in target_pastures:
                if px < len(farm["tiles"][0]) and py < len(farm["tiles"]):
                    t = farm["tiles"][py][px]
        ...
        if current_tile != "LOCKED" and (cx, cy) not in target_pastures:
            if isinstance(current_tile, dict):
                ...
                seeds = private.get("seeds", {})
                # Strictly plant high value first: STRAWBERRY -> MELON -> WHEAT
                for c in ["STRAWBERRY", "MELON", "WHEAT", "CARROT", "TOMATO"]:
                    if seeds.get(c, 0) > 0:
                        return ["PLANT", c]

        # Phase 7: Deposit to Shed in evening or when carrying produce
...
def agent(obs):
    ...
    if obs.get("step", 0) == 0 or p_id not in _player_agents:
        _player_agents[p_id] = ExactGrandmasterAgent()
