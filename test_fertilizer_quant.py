"""
Test Fertilizer Integration with Quant Grandmaster:
- Buy Fertilizer on Days 6-12 when Melons enter their watering window.
- Apply FERTILIZE op on Melon tiles.
- Melon watering bonus increases from +1 to +2 per day for 3 days (+3 extra yield = +$750 revenue per tile!).
"""
import math
...
def agent(obs):
    ...
    if _agent_inst is None or obs.get("step", 0) == 0:
        _agent_inst = FertilizerQuantAgent()
