"""
Grandmaster Phased Agricultural Agent:
- Phase 1 (Days 0-4): Rapid Cash & Wheat reserve accumulation
- Phase 2 (Days 4-10): Land expansion + Livestock sanctuary deployment with guaranteed feed
- Phase 3 (Days 10-25): Daily Care + Milk/Wool/Fertilizer harvesting + Fertilized Strawberry/Melon crops
- Phase 4 (Days 26-30): Season-end liquidation
"""
import math
...
# Pastures clustered right around the shed in NW quadrant
SANCTUARY_TILES = [(3, 4), (4, 3), (3, 3), (2, 4)]
...
class PhasedGrandmasterAgent:
    ...
    def act(self, obs):
        ...
        if hour == 0:
            ...
            # Animal Purchase (Only after Day 4 when wheat is harvested and stored in shed)
            wheat_in_shed = shed.get("WHEAT", 0)
            if day >= 4 and wheat_in_shed >= 8 and active_animals < len(SANCTUARY_TILES) and avail_money >= 600:
                cows_in_shed = shed.get("COW", 0)
                if (active_animals + cows_in_shed) < len(SANCTUARY_TILES):
                    buy_cows = min(len(SANCTUARY_TILES) - (active_animals + cows_in_shed), int((avail_money * 0.4) // 400))
                    if buy_cows > 0 and len(market_orders) < 10:
                        market_orders.append(["BUY_ANIMAL", "COW", buy_cows])
                        avail_money -= buy_cows * 400

            # Guaranteed Wheat Patch for animal feed (at least 6 wheat seeds planted)
            if day <= 16 and (seeds.get("WHEAT", 0) + wheat_in_shed) < 12 and avail_money >= 60:
                buy_w = min(6, int(avail_money // 10))
            ...
            # High Value Melon Quota (Days 0-14)
            if day <= 14 and avail_money >= 160:
                ...
                target_melons = 12 if num_tiles >= 50 else 6
        ...
        for item, count in shed.items():
            if count > 0 and item in PRODUCTS:
                ...
                if item == "WHEAT" and day < 27:
                    # Reserve 10 wheat in shed for cows
                    sell_count = max(0, count - 10)
        ...
        for w_idx, w_pos in enumerate(all_workers):
            ...
            if w_idx == 0 and num_active > 1 and day >= 4:
                # Dedicated Sanctuary Caretaker starting Day 4
    ...
    def _caretaker_act(self, pos, sanctuary_tiles, farm, private, inv, day, hour):
        ...
        if (cx, cy) in SHED_ACCESS:
            if shed.get("COW", 0) > 0 and inv.get("COW", 0) == 0:
                return ["PICKUP", "COW", 1]
            if shed.get("WHEAT", 0) > 0 and inv.get("WHEAT", 0) < 4:
                return ["PICKUP", "WHEAT", min(4, shed.get("WHEAT", 0))]
...
def agent(obs):
    ...
    if agent_instance is None or obs.get("step", 0) == 0:
        agent_instance = PhasedGrandmasterAgent()
