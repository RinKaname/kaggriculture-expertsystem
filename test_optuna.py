...
# --- FULLY PARAMETERIZED AGENT ---
class FullyParameterizedApexAgent:
...
def run_optimization():
    ...
    def objective(trial):
        # 1. Package all 16 categorical parameters directly into a single dictionary
        params = {
            # --- Original 4 Parameters ---
            "sell_thresh": trial.suggest_categorical("sell_thresh", [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.90]),
            "straw_target": trial.suggest_categorical("straw_target", [20, 35, 40, 45, 50, 55, 60]),
            "melon_target": trial.suggest_categorical("melon_target", [5, 6, 7, 8, 9, 10, 15, 20]),
            "melon_cutoff_day": trial.suggest_categorical("melon_cutoff_day", [12, 15, 17, 18, 19, 20, 22]),
            
            # --- 1. Livestock Knobs ---
            "max_animals_quad2": trial.suggest_categorical("max_animals_quad2", [8, 10, 12, 14, 16]),
            "animal_cutoff_day": trial.suggest_categorical("animal_cutoff_day", [10, 12, 14, 16, 18]),
            "animal_min_cash": trial.suggest_categorical("animal_min_cash", [600, 750, 900, 1000, 1200]),
            "feed_buffer_per_animal": trial.suggest_categorical("feed_buffer_per_animal", [2, 3, 4, 5]),

            # --- 2. Labor Knobs ---
            "tiles_per_worker": trial.suggest_categorical("tiles_per_worker", [4.0, 4.5, 5.0, 5.2, 5.5, 6.0]),
            "animal_keepers": trial.suggest_categorical("animal_keepers", [1, 2, 3]),
            "hire_cutoff_hour": trial.suggest_categorical("hire_cutoff_hour", [2, 4, 6, 8]),

            # --- 3. Land Expansion Timing ---
            "quad2_day_cutoff": trial.suggest_categorical("quad2_day_cutoff", [6, 8, 10, 12]),
            "quad3_day_cutoff": trial.suggest_categorical("quad3_day_cutoff", [12, 14, 16, 18, 20]),
            "quad4_enable": trial.suggest_categorical("quad4_enable", [True, False]),

            # --- 4. Market & Pacing Knobs ---
            "milk_wool_thresh": trial.suggest_categorical("milk_wool_thresh", [0.40, 0.45, 0.50, 0.55, 0.60, 0.70]),
            "sell_batch_size": trial.suggest_categorical("sell_batch_size", [3, 4, 5, 6, 8, 10]),
            "emergency_shed_cap": trial.suggest_categorical("emergency_shed_cap", [50, 60, 65, 70, 75, 80]),
            "endgame_liquidation_day": trial.suggest_categorical("endgame_liquidation_day", [26, 27, 28, 29]),

            # --- 5. Crop Timing ---
            "strawberry_start_day": trial.suggest_categorical("strawberry_start_day", [2, 4, 6, 8]),
            "fert_start_day": trial.suggest_categorical("fert_start_day", [6, 8, 10, 12])
        }

        # 2. Pass the entire parameter dictionary to the wrapper
        def agent_wrapper(obs):
            if obs.get("step", 0) == 0:
                agent_wrapper.instance = FullyParameterizedApexAgent(params)
            return agent_wrapper.instance.act(obs)

        # 3. Run multiple games to get a stable average reward against starter
        num_games = 3
        total_reward = 0
        for _ in range(num_games):
            _ = env.run([agent_wrapper, "starter"])
            final_state = env.steps[-1]
            my_reward = final_state[0].reward
            total_reward += my_reward if my_reward is not None else 0
    ...
    # Wrap the ask/tell loop in tqdm to give estimated time exactly how you like it
    with tqdm(total=n_trials, desc="Tuning Agent", unit="trial") as pbar:
        for _ in range(n_trials):
            trial = study.ask()
            reward = objective(trial)
            study.tell(trial, reward)
            
            # Update the progress bar postfix with the best score found so far
