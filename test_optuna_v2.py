...
TOWN_SHOPS = {
    "Bakery": ["WHEAT", "STRAWBERRY", "CARROT"],
    "Ice Cream Parlor": ["MILK", "STRAWBERRY"],
    "Textile Mill": ["WOOL"],
    "Pizzeria": ["WHEAT", "TOMATO"],
    "Juice Bar": ["CARROT", "MELON", "STRAWBERRY"],
    "Salad Bar": ["CARROT", "TOMATO"],
    "Diner": ["EGG", "MILK", "WHEAT", "CARROT"],
    "Cafe": ["MILK", "EGG", "STRAWBERRY"],
}
...
# --- ADAPTIVE APEX GRANDMASTER AGENT ---
class AdaptiveApexAgent:
    ...
    def get_shop_demand(self, unlocked_shops):
        ...
        for shop in unlocked_shops:
            ...
            for it in items:
                demand[it] += (2 if (shop == "Textile Mill" and it == "WOOL") else 1)
        return demand
    ...
    def act(self, obs):
        ...
        # --- 1. Dynamic Adaptive Market Execution ---
        ...
        for item, count in shed.items():
            ...
            # Shop Cadence: If step % 4 == 0 (consumption tick), allow larger batch
            is_shop_tick = (step % 4 == 0) and (shop_demand.get(item, 0) > 0)
            batch_size = (self.p["sell_batch_size"] + self.p["shop_batch_bonus"]) if is_shop_tick else self.p["sell_batch_size"]

            if is_emergency_dump or cur_p >= (base_p * thresh):
                sell_qty = count if (is_emergency_dump or item == "FERTILIZER" or day >= 28) else min(count, batch_size)
        ...
        # --- 6. Adaptive Seed Economy & Crop Matrix ---
    ...
    def _get_worker_action(self, pos, assigned_tiles, target_pastures, farm, private, day, hour, inv, worker_idx):
        ...
        # Phase 1: Structure Building (Clear Weed first if present)
        ...
        # Phase 3: Shed Operations (Deposit Goods & Refill Wheat/Fertilizer)
        if is_shed_adj:
        ...
        # Phase 8: Move to urgent crop/weed tile
        target_tile = None
...
# --- OPTUNA V2 OPTIMIZATION LOOP ---
def run_optimization():
    env = make("kaggriculture", configuration={"episodeSteps": 720}, debug=False)
    
    def objective(trial):
        params = {
            # Pricing & Town Shop Synergy
    ...
    print("Starting Adaptive V2 Bayesian Optimization...")
    with tqdm(total=n_trials, desc="Tuning Adaptive V2", unit="trial") as pbar:
