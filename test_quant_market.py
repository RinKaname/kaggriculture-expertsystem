"""
Quantitative Market Timing & Fertilizer Integration:
1. Dynamic Paced Selling:
   - Hold inventory in shed when market is flooded (price below floor threshold).
   - Sell in small batches when town consumption drives prices up above target.
   - Force-sell when shed capacity exceeds 85/100 or on Day >= 29.
2. Dynamic Town Demand Adaptation:
   - Check unlocked shops and boost planting of crops currently demanded by active shops.
"""
import math
...
SHOPS = {
    "Bakery": ["WHEAT", "EGG", "MILK"],
    "Juice Bar": ["CARROT", "TOMATO", "MELON"],
    "Ice Cream Parlor": ["STRAWBERRY", "MILK", "EGG"],
    "Pizzeria": ["TOMATO", "WHEAT", "MILK"],
    "Wool Boutique": ["WOOL"],
    "Smoothie Stand": ["STRAWBERRY", "MELON", "CARROT"],
    "Brunch Cafe": ["EGG", "WHEAT", "STRAWBERRY"],
    "Farmers Market": ["CARROT", "TOMATO", "STRAWBERRY", "MELON"],
}
...
class QuantAgent:
    ...
    def act(self, obs):
        ...
        if hour == 0:
            # 1. Disciplined Land Expansion: NE ($1k) Day 0-4, SW ($2k) Day 4-10, SE ($4k) Day 8-14
            if unlocked_quads == 1 and money >= 1100 and day <= 5:
                market_orders.append(["BUY_LAND"])
            elif unlocked_quads == 2 and money >= 2200 and day <= 10:
                market_orders.append(["BUY_LAND"])
            elif unlocked_quads == 3 and money >= 4500 and day <= 14:
                market_orders.append(["BUY_LAND"])
            ...
            if day <= 16 and avail_money >= 160:
                melon_target = 35 if num_tiles >= 75 else (20 if num_tiles >= 50 else 10)
            ...
            if day <= 14 and avail_money >= 200:
                straw_target = 15 if num_tiles >= 75 else 8
        ...
        for item, count in shed.items():
            if count <= 0 or item not in PRODUCTS:
                continue
            if item == "FERTILIZER":
                continue  # fertilizer cannot be sold

            cur_p = prices.get(item, 1)
            base_p = CROPS.get(item, {}).get("base_price", 50)
            
            # Price threshold: sell if current price is >= 70% of base, or if emergency dump
            if is_emergency_dump or cur_p >= (base_p * 0.70):
                # Metered batch selling: sell up to 4 units at high price to avoid crashing market in 1 turn
