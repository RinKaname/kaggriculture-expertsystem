...
def replica_agent(obs, config=None):
    global _PENDING_PASTURE, _FARMER_SHIFT_END
    global _PENDING_PLANT, _PENDING_WATER, _WATER_SHIFT, _REPAIR_ACTIVATED
    step=int(obs.get("step", 0) or 0)
    if step == 0:
...
if __name__ == "__main__":
    print("Testing Replica vs Starter, Baseline, and Main...")
    
    # 1. vs Starter
    env = make("kaggriculture", configuration={"episodeSteps": 720}, debug=False)
    env.run([replica_agent, "starter"])
    p0_score = env.steps[-1][0]["reward"] or 0
    print(f"Replica vs Starter: ${p0_score:,.2f}")

    # 2. vs Baseline
    env = make("kaggriculture", configuration={"episodeSteps": 720}, debug=False)
    env.run([replica_agent, baseline.agent])
    r_score = env.steps[-1][0]["reward"] or 0
    b_score = env.steps[-1][1]["reward"] or 0
    print(f"Replica (${r_score:,.2f}) vs Baseline (${b_score:,.2f})")

    # 3. vs Main
    env = make("kaggriculture", configuration={"episodeSteps": 720}, debug=False)
    env.run([replica_agent, main.agent])
    r_score2 = env.steps[-1][0]["reward"] or 0
    m_score2 = env.steps[-1][1]["reward"] or 0
    print(f"Replica (${r_score2:,.2f}) vs Main (${m_score2:,.2f})")
